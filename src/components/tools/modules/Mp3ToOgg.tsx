...
import { Music2, Upload, Download, Loader2 } from 'lucide-react';
...
export default function Mp3ToOgg() {
  ...
  const processAudio = async () => {
    ...
    try {
      ...
      await ffmpeg.exec(['-i', inputName, '-c:a', 'libvorbis', '-qscale:a', '5', outputName]);
    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-8 rounded-2xl shadow-xl space-y-6 text-center">
         <div className="flex items-center justify-center gap-3 mb-2">
           ...
           <h2 className="text-2xl font-bold">MP3 to OGG Converter</h2>
         </div>
         <p className="text-zinc-500">Convert MP3 audio files into web-friendly OGG format.</p>
           ...
           <div className="mt-8 space-y-6 text-left">
             ...
             {isLoaded && !outputUrl && !isProcessing && (
               <button 
                 ...
                 className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95"
               >
                 Convert to OGG
               </button>
             )}
             ...
             {isProcessing && (
               <div className="space-y-2">
                 ...
                 <div className="w-full bg-zinc-200 dark:bg-zinc-800 rounded-full h-3 overflow-hidden">
                   <div className="bg-orange-500 h-full transition-all duration-300 ease-out" style={{ width: `${progress}%` }}></div>
                 </div>
               </div>
             )}
             ...
             {outputUrl && (
               <div className="space-y-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                 ...
                 <a 
                   href={outputUrl} 
                   download={`${file.name.replace(/\.[^/.]+$/, "")}.ogg`}
