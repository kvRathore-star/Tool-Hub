...
export default function AiHashtagGenerator() {
  ...
  const [quantity, setQuantity] = useState("20 hashtags");

  const handleGenerate = async () => {
    if (!content.trim()) return toast.error('Please fill in the Post Caption / Content Topic field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a professional social media marketing manager.\n\nGenerate ${quantity} relevant hashtags for a post on ${platform} about: ${content}. Group them by popularity/reach category (High Reach, Medium Reach, Niche/Targeted).`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "hashtags_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Hashtag Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate highly relevant and optimized social media hashtags.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Post Caption / Content Topic</label>
              <textarea
                value={content}
                onChange={e => setContent(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleGenerate()}
                placeholder="e.g., Launching my new digital marketing agency for local shops..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Target Platform</label>
              <select
                value={platform}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Hashtag Count</label>
              <select
                value={quantity}
                onChange={e => setQuantity(e.target.value)}
                className="w-full bg-zinc-50 dark:bg-black/50 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 text-zinc-900 dark:text-white outline-none focus:border-zinc-300 dark:focus:border-zinc-700 transition-colors text-sm"
              >
                <option value="10 hashtags">10 hashtags</option>
                <option value="20 hashtags">20 hashtags</option>
                <option value="30 hashtags">30 hashtags</option>
              </select>
            </div>
          </div>
