...
export default function AiRegexGenerator() {
  ...
  const handleGenerate = async () => {
    if (!description.trim()) return toast.error('Please fill in the Regex Match Description field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are an expert developer. Generate regex patterns and explain them.\n\nCreate a regular expression for: ${description}. Test strings for context: ${testStrings}. Output the regex pattern, flags, and a brief explanation.`;
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = "regex_" + new Date().toISOString().slice(0,10) + ".txt";
    link.click();
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Regex Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate regular expressions from description and test strings.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Regex Match Description</label>
              <textarea
                value={description}
                onChange={e => setDescription(e.target.value)}
                placeholder="e.g., Match a secure password with at least 8 characters, one number, and one uppercase letter..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Test Cases / Examples (Optional)</label>
              <textarea
                value={testStrings}
                onChange={e => setTestStrings(e.target.value)}
                placeholder="e.g., Pass123! (should match)
12345 (should not match)"
          ...
          <button 
            ...
            className="mt-6 w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-amber-500 to-orange-600 hover:shadow-amber-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg"
          >
            {isProcessing ? (
              <>
