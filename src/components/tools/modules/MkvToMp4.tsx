...
export default function MkvToMp4() {
  ...
  const handleFileSelect = (selectedFile: File) => {
    if (!selectedFile.name.toLowerCase().endsWith('.mkv')) {
      toast.error("Please upload an .mkv file.");
    }
  ...
  const convertVideo = async () => {
    ...
    try {
      ...
      toast.success("Converted MKV to MP4 successfully!");
    } catch (e) {
      console.error(e);
    ...
    return (
      <div className="space-y-6 max-w-3xl mx-auto animate-in fade-in duration-500">
        <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl text-blue-500 text-sm">
          <strong>MKV to MP4 Converter:</strong> Transcode Matroska (.mkv) video files into universal .MP4 files securely inside your browser. No files are uploaded.
        </div>
        <FileUploader 
          accept=".mkv,video/x-matroska"
          onFileSelect={handleFileSelect} 
          title="Upload MKV Video"
          subtitle="Select a .mkv file to convert"
        />
      </div>
    );
  }
          ...
          <div className="text-center space-y-4">
             <div className="flex justify-center items-center gap-4 text-zinc-400">
                <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl">
                  <span className="font-black text-xl text-zinc-800 dark:text-zinc-200">.MKV</span>
                </div>
