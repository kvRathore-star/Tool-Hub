...
export default function AiPoemGenerator() {
  ...
  const [mood, setMood] = useState("Reflective & Quiet");

  const handleGenerate = async () => {
    if (!topic.trim()) return toast.error('Please fill in the Poem Topic or Theme field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a talented poet.\n\nWrite a poem about: ${topic} in the style of a ${style} with a ${mood} mood.`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "poem_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Poem Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Write custom poems in various styles and moods.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Poem Topic or Theme</label>
              <textarea
                value={topic}
                onChange={e => setTopic(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleGenerate()}
                placeholder="e.g., The sound of rain on a tin roof at midnight..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Poetic Style</label>
              <select
                value={style}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Mood</label>
              <select
                value={mood}
                onChange={e => setMood(e.target.value)}
                className="w-full bg-zinc-50 dark:bg-black/50 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 text-zinc-900 dark:text-white outline-none focus:border-zinc-300 dark:focus:border-zinc-700 transition-colors text-sm"
              >
                <option value="Reflective & Quiet">Reflective & Quiet</option>
                <option value="Melancholic & Sad">Melancholic & Sad</option>
                <option value="Joyful & Energetic">Joyful & Energetic</option>
                <option value="Mystical & Dreamy">Mystical & Dreamy</option>
