...
export default function AiChangelogGenerator() {
  ...
  const [tone, setTone] = useState("User-friendly & Simple");

  const handleGenerate = async () => {
    if (!commits.trim()) return toast.error('Please fill in the Commit Logs or Updates List field');
    if (!version.trim()) return toast.error('Please fill in the Version Number field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a product manager.\n\nCreate a changelog for version ${version} based on these commit logs: ${commits}. Vibe/Style: ${tone}. Format using standard markdown headers (Added, Changed, Fixed, Removed).`;
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = "changelog_" + new Date().toISOString().slice(0,10) + ".txt";
    link.click();
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Changelog Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Convert commit logs or git history into clean user changelogs.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Commit Logs or Updates List</label>
              <textarea
                value={commits}
                onChange={e => setCommits(e.target.value)}
                placeholder="e.g., feat: Add dark mode toggle
fix: Fix crash in file downloader
chore: bump react-cropper..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Version Number</label>
              <input
                type="text"
                value={version}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Changelog Style</label>
              <select
                value={tone}
                onChange={e => setTone(e.target.value)}
                className="w-full bg-zinc-50 dark:bg-black/50 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 text-zinc-900 dark:text-white outline-none focus:border-zinc-300 dark:focus:border-zinc-700 transition-colors text-sm"
              >
                <option value="User-friendly & Simple">User-friendly & Simple</option>
                <option value="Developer-focused / Technical">Developer-focused / Technical</option>
          ...
          <button 
            ...
            className="mt-6 w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-zinc-600 to-slate-800 hover:shadow-zinc-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg"
          >
            {isProcessing ? (
              <>
