...
export default function AiExcelFormulaGenerator() {
  const { isConfigured, generateCompletion } = useAiProvider();
  const [isProcessing, setIsProcessing] = useState(false);
  const [outputText, setOutputText] = useState('');
  
  const [description, setDescription] = useState("");
  const [software, setSoftware] = useState("Microsoft Excel");

  const handleGenerate = async () => {
    if (!description.trim()) return toast.error('Please fill in the Formula Requirement field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are an expert spreadsheet consultant. Generate formulas and explain their usage.\n\nCreate a formula for ${software} to: ${description}. Provide the formula in a code block, then explain how it works and what columns it uses.`;
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = "formula_" + new Date().toISOString().slice(0,10) + ".txt";
    link.click();
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Excel Formula Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate spreadsheet formulas for Excel or Google Sheets.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Formula Requirement</label>
              <textarea
                value={description}
                onChange={e => setDescription(e.target.value)}
                placeholder="e.g., Check if cell A2 is in column B, and return the corresponding value in column C, otherwise return 'Not Found'..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Spreadsheet Software</label>
              <select
                value={software}
