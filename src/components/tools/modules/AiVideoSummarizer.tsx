...
export default function AiVideoSummarizer() {
  ...
  const [format, setFormat] = useState("Bulleted Key Takeaways");

  const handleGenerate = async () => {
    if (!transcript.trim()) return toast.error('Please fill in the Video Transcript or text notes field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are an expert research assistant and content analyst.\n\nProvide a detailed summary of the following video content. Summary format: ${format}. Transcript/Text:
${transcript}`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "video_summary_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Video Summarizer</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Summarize video content, transcripts, or video texts quickly.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Video Transcript or text notes</label>
              <textarea
                value={transcript}
                onChange={e => setTranscript(e.target.value)}
                placeholder="Paste video transcripts or notes here..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Summary Structure</label>
              <select
                value={format}
              ...
              >
                <option value="Bulleted Key Takeaways">Bulleted Key Takeaways</option>
                <option value="Executive Summary">Executive Summary</option>
                <option value="Detailed Section Outline">Detailed Section Outline</option>
              </select>
            </div>
          </div>
          ...
          <button 
            ...
            className="mt-6 w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-red-600 to-orange-600 hover:shadow-red-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg"
          >
            {isProcessing ? (
              <>
