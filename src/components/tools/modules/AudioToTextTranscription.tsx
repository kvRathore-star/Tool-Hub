...
${audioText}`;
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = "transcription_clean_" + new Date().toISOString().slice(0,10) + ".txt";
    link.click();
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">Audio to Text Transcription Helper</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Clean up and format raw audio transcriptions into structured documents.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Raw Transcript Text</label>
              <textarea
                value={audioText}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Clean Up Style</label>
              <select
                value={format}
