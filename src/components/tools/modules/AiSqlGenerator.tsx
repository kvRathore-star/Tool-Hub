...
export default function AiSqlGenerator() {
  ...
  const handleGenerate = async () => {
    if (!request.trim()) return toast.error('Please fill in the Query Requirement field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are an expert database administrator. Generate optimized SQL queries.\n\nGenerate a ${dialect} query for: ${request}. Schema context (if any): ${schema}. Output ONLY the SQL code inside a markdown block.`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "query_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI SQL Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Convert natural language descriptions into optimized SQL queries.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Query Requirement</label>
              <textarea
                value={request}
                onChange={e => setRequest(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleGenerate()}
                placeholder="e.g., Select all customers who placed an order in the last 30 days and spent more than $100 in total..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Table Schema Context (Optional)</label>
              <textarea
                value={schema}
                onChange={e => setSchema(e.target.value)}
                placeholder="e.g. Table: customers (id, name, email); Table: orders (id, customer_id, amount, date)..."
                className="w-full bg-zinc-50 dark:bg-black/50 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 text-zinc-900 dark:text-white h-32 outline-none focus:border-zinc-300 dark:focus:border-zinc-700 transition-colors text-sm resize-none"
              />
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">SQL Dialect</label>
              <select
                value={dialect}
          ...
          <button 
            ...
            className="mt-6 w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 hover:shadow-indigo-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg"
          >
            {isProcessing ? (
              <>
