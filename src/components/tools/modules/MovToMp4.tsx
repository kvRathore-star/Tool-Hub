  ...
  const convertVideo = async () => {
    ...
    try {
      ...
      await ffmpeg.exec([
        ...
        '-preset', 'fast', 
        '-pix_fmt', 'yuv420p', // Ensure maximum compatibility across browsers/devices
        'output.mp4'
      ]);
    ...
    return (
      <div className="space-y-6 max-w-3xl mx-auto animate-in fade-in duration-500">
        <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl text-blue-500 text-sm">
          <strong>Apple QuickTime Converter:</strong> Convert .MOV files (usually from iPhones or Macs) into universal .MP4 files securely inside your browser.
        </div>
        <FileUploader 
          accept=".mov,video/quicktime"
          onFileSelect={handleFileSelect} 
          title="Upload MOV Video"
          subtitle="Select a .mov file to convert"
        />
      </div>
    );
  }
          ...
          <div className="text-center space-y-4">
             <div className="flex justify-center items-center gap-4 text-zinc-400">
                <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl">
                  <span className="font-black text-xl text-zinc-800 dark:text-zinc-200">.MOV</span>
                </div>
