        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-5 rounded-2xl shadow-xl space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleCopy} className="bg-zinc-800 px-3 py-2.5 rounded-xl text-white cursor-pointer" aria-label="Copy"><Copy className="w-4 h-4" /></button>
            </div>
            <div className="flex gap-2">
              <button onClick={generateEmail} className="flex-1 bg-[var(--bg-overlay)] dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-[var(--text-secondary)] dark:text-zinc-200 font-bold py-2 rounded-xl text-[10px] cursor-pointer flex justify-center items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5" /> Reset Mail</button>
            ...
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
              {inbox.map(mail => (
                <div 
                  ...
                  className={`p-3 border rounded-xl cursor-pointer hover:border-[var(--border-subtle)] transition-colors ${
                    selectedMail?.id === mail.id 
                      ? 'bg-indigo-500/10 border-indigo-500' 
                      : 'bg-zinc-50 dark:bg-black/10 border-[var(--border-subtle)]'
                  }`}
                >
          ...
          {selectedMail ? (
            <div className="space-y-4 animate-in zoom-in-95 flex-1 flex flex-col justify-between">
              <div className="border-b border-zinc-800 pb-3">
                <p className="font-bold text-zinc-300">From: <span className="font-normal text-[var(--text-muted)]">{selectedMail.from}</span></p>
                <p className="font-bold text-zinc-300 mt-1">Subject: <span className="font-normal text-[var(--text-muted)]">{selectedMail.subject}</span></p>
              </div>
              <div className="flex-1 bg-zinc-50 dark:bg-black/50 border border-[var(--border-subtle)] rounded-xl p-4 font-sans text-zinc-200 leading-relaxed overflow-y-auto h-52">
                {selectedMail.body}
              </div>
            </div>
          ) : (
