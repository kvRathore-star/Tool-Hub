...
export default function AiRecipeGenerator() {
  ...
  const handleGenerate = async () => {
    if (!ingredients.trim()) return toast.error('Please fill in the Ingredients You Have field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a professional chef. Create delicious, easy-to-follow recipes.\n\nCreate a custom recipe for a ${mealType} using these ingredients: ${ingredients}. Dietary restrictions: ${restrictions}. Include: Recipe Title, prep/cook times, full ingredients list, step-by-step instructions, and chef's tips.`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "recipe_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Recipe Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate custom recipes based on ingredients and diet restrictions.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Ingredients You Have</label>
              <textarea
                value={ingredients}
                onChange={e => setIngredients(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleGenerate()}
                placeholder="e.g. Chicken breasts, cherry tomatoes, spinach, garlic, pasta, olive oil..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Meal Type</label>
              <select
                value={mealType}
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Dietary Restrictions</label>
              <select
                value={restrictions}
          ...
          <button 
            ...
            className="mt-6 w-full py-4 rounded-xl font-bold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:shadow-emerald-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-md hover:shadow-lg"
          >
            {isProcessing ? (
              <>
