...
export default function AiSloganGenerator() {
  ...
  const [vibe, setVibe] = useState("Creative & Witty");

  const handleGenerate = async () => {
    if (!description.trim()) return toast.error('Please fill in the Business or Product Description field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a professional brand strategist and copywriter.\n\nGenerate 15 unique, catchy brand slogans/taglines for: ${description}. Keywords: ${keywords}. Vibe: ${vibe}.`;
  ...
  const handleDownload = () => {
    ...
    downloadOrShare(url, "slogans_" + new Date().toISOString().slice(0,10) + ".txt");
  };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Slogan Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate memorable slogans and taglines for your brand.</p>
            
<div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Business or Product Description</label>
              <textarea
                value={description}
                onChange={e => setDescription(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleGenerate()}
                placeholder="e.g., A subscription service delivering organic, roast-on-demand coffee beans..."
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Keywords to Include (Optional)</label>
              <input
                type="text"
                value={keywords}
                onChange={e => setKeywords(e.target.value)}
                placeholder="e.g., fresh, awaken, fairtrade"
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Vibe / Tone</label>
              <select
                value={vibe}
              ...
              >
                ...
                <option value="Trustworthy & Classic">Trustworthy & Classic</option>
                <option value="Playful & Fun">Playful & Fun</option>
                <option value="Bold & Premium">Bold & Premium</option>
              </select>
            </div>
          </div>
