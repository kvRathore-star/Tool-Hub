...
export default function AiBusinessIdeaGenerator() {
  ...
  const [budget, setBudget] = useState("Under $1,000 (Lean)");
  const [timeCommit, setTimeCommit] = useState("Side Hustle (5-10 hrs/wk)");

  const handleGenerate = async () => {
    if (!interests.trim()) return toast.error('Please fill in the Your Skills, Passions, or Industry Interest field');
    if (!isConfigured) return toast.error('Please configure your AI Provider API key at the top first!');

    setIsProcessing(true);
    try {
      const prompt = `You are a startup founder, entrepreneur, and venture builder.\n\nGenerate 3 startup business ideas based on: Interests: ${interests}, Startup Budget: ${budget}, Time Commitment: ${timeCommit}. For each idea, output: Business Name, One-sentence Hook, Target Customer, Monetization Strategy, and a Step-by-Step launch plan.`;
        ...
        <div className="lg:col-span-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">AI Business Idea Generator</h3>
            </div>
            
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-4">Generate detailed startup business ideas tailored to your budget and interests.</p>
...
<div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Available Budget</label>
              <select
                value={budget}
              ...
              >
                <option value="Under $1,000 (Lean)">Under $1,000 (Lean)</option>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Time Commitment</label>
              <select
                value={timeCommit}
              ...
              >
                ...
                <option value="Full-Time Venture">Full-Time Venture</option>
              </select>
            </div>
          </div>
